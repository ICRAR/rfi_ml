...
int main(int argc, char * const argv[]) {
  ...
  if (mode==MARK5B) {
    bufsize=MK5BFRAMESIZE+MK5BHEADSIZE;
    filesize = bufsize;
    init_bitreversal();
    ...
    for(i=0;i<numthread;i++) {
      ...
      status = vdif_setmjdsec(&vdif_headers[i], mjd*60*60*24);
      if (status!=VDIF_NOERROR) {
  ...
  if (mode==LBADR) {
    ...
    header = NULL; 
    strcpy(buf+MK5BHEADSIZE, "Chris was here");
  } else if (mode==VDIF) {
  ...
  while (mjd+0.001/60/60/24<finishmjd) {
    ...
    if (mode==MARK5B) {
      ptr = buf+udp.extra;
      memcpy(ptr, mk5_header.header, MK5BHEADSIZE); // Inefficent, but oh well
