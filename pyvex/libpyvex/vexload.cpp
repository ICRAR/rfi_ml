 ...
 *
 * $Id: vexload.cpp 8614 2018-12-18 15:16:30Z JanWagner $
 * $HeadURL: https://svn.atnf.csiro.au/difx/applications/vex2difx/trunk/vexdatamodel/vexload.cpp $
 * $LastChangedRevision: 8614 $
 * $Author: JanWagner $
 * $LastChangedDate: 2018-12-18 23:16:30 +0800 (Tue, 18 Dec 2018) $
 *
 *==========================================================================*/
...
double vexDate(char *value)
{
	...
	else
	{
		...
		mjd = DOYtoMJD( static_cast<int>(std::floor(years+0.1)), static_cast<int>(std::floor(days+0.1)) ) + hours/24.0 + minutes/1440.0 + seconds/86400.0;
	}
	...
	for(const char *modeDefName = get_mode_def(v); modeDefName; modeDefName = get_mode_def_next())
	{
		...
		for(unsigned int a = 0; a < V->nAntenna(); ++a)
		{
			...
			for(p = get_all_lowl(antName.c_str(), modeDefName, T_IF_DEF, B_IF, v); p; p = get_all_lowl_next())
			{
				...
				if(std::fabs(phaseCal) < 1.0)
				{
					vif.phaseCalIntervalMHz = 0.0f;
				}
				else if(std::fabs(phaseCal-1000000.0) < 1.0)
				{
					vif.phaseCalIntervalMHz = 1.0f;
				}
				else if(std::fabs(phaseCal-5000000.0) < 1.0)
				{
					vif.phaseCalIntervalMHz = 5.0f;
				}
				else if(std::fabs(phaseCal-200000000.0) < 1.0)
				{
